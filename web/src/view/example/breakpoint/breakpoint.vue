<template>
  <div class="hello">
    ...
    <div
      class="tips"
    >此版本为先行体验功能测试版，样式美化和性能优化正在进行中，上传切片文件和合成的完整文件分别再QMPlusserver目录的breakpointDir文件夹和fileDir文件夹</div>
  </div>
</template>
<script>
  ...
  methods: {
      ...
      if (file.size < maxSize) {
        ...
        fileR.onload = async (e) => {
          ...
          while (end < file.size) {
            ...
            let fileSlice = this.file.slice(start, end); // 开始切  file.slice 为 h5方法 对文件切片 参数为 起止字节数
    ...
    getFile() {
      ...
      this.percentage = Math.floor(
        ((this.formDataList.length - this.waitNum) / this.formDataList.length) *
          100
      );
