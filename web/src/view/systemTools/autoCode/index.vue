  ...
  <div>
    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item name="1">
        ...
        <!-- 数据库连接 -->
        <el-form ref="getTableForm" :inline="true" :model="dbform" label-width="90px">
          <el-form-item label="数据库连接" prop="structName">
            <el-select
              @change="getDB"
              v-model="dbform.connectType"
              :style="{width: '135px'}"
              filterable
              placeholder="请选择连接"
            >
              <el-option
                ...
                :key="index"
                :label="`${item.name}(${item.dbName})`"
                :value="item.connectType"
              ></el-option>
            </el-select>
          </el-form-item>
          <!-- 数据库名 -->
          <el-form-item label="数据库名" prop="structName">
            <el-select
              ...
              :disabled="dbform.connectType===undefined"
              filterable
              placeholder="请选择数据库"
              :style="{width: '135px'}"
            >
...
export default {
  ...
  methods: {
    ...
    async getDBConnect() {
      ...
      if (code == 0) {
        this.connectOptions = data.connects;
      }
    },
    async getDB() {
      console.error(typeof this.dbform.connectType);
      const res = await getDB({ connectType: this.dbform.connectType });
      if (res.code == 0) {
        this.dbOptions = res.data.dbs;
      }
      console.error({
        method: "getDB",
        connectType: this.dbform.connectType,
        dbOptions: this.dbOptions,
      });
    },
    async getTable() {
      console.error({ method: "getTable", data: this.dbform });
