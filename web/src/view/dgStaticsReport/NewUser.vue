<template>
  <div class="chartContainer">
    ...
    >
      ...
      <!-- <el-form-item label="选择日期" class="datePicker">
        <el-col :span="11">
          <el-date-picker
...
import { newUser } from "@/api/daigou/staticsReport";
...
const legendName = ["日新增用户数", "周新增用户数", "月新增用户数"];
export default {
  name: "NewUser",
  components: {
    "v-chart": ECharts,
  ...
  methods: {
    ...
    async onGetData(formName) {
      this.$refs[formName].validate(async (valid) => {
        ...
        //发生api请求
        const { code, data, msg } = await newUser({
