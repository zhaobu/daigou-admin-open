...
// @Param data body sys_model.SysAuthority true "创建角色"
...
func CreateAuthority(c *gin.Context) {
	...
	if AuthorityVerifyErr != nil {
		respwrite.FailWithMessage(AuthorityVerifyErr.Error(), c)
		return
	}
	err, authBack := sys_service.CreateAuthority(auth)
	if err != nil {
		respwrite.FailWithMessage(fmt.Sprintf("创建失败，%v", err), c)
	} else {
		respwrite.OkWithData(response.SysAuthorityResponse{Authority: authBack}, c)
	}
}
...
// @accept application/json
// @Produce application/json
// @Param data body respwrite.SysAuthorityCopyResponse true "拷贝角色"
...
func CopyAuthority(c *gin.Context) {
	...
	if OldAuthorityVerifyErr != nil {
		respwrite.FailWithMessage(OldAuthorityVerifyErr.Error(), c)
		return
	}
	AuthorityVerify := utils.Rules{
		"AuthorityId":   {utils.NotEmpty()},
	...
	err, authBack := sys_service.CopyAuthority(copyInfo)
	if err != nil {
		respwrite.FailWithMessage(fmt.Sprintf("拷贝失败，%v", err), c)
	} else {
		respwrite.OkWithData(response.SysAuthorityResponse{Authority: authBack}, c)
	}
}
...
// @accept application/json
// @Produce application/json
// @Param data body sys_model.SysAuthority true "删除角色"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"获取成功"}"
...
func DeleteAuthority(c *gin.Context) {
	...
	err := sys_service.DeleteAuthority(&a)
	if err != nil {
...
// @accept application/json
// @Produce application/json
// @Param data body sys_model.SysAuthority true "设置角色资源权限"
...
func UpdateAuthority(c *gin.Context) {
	...
	err, authority := sys_service.UpdateAuthority(auth)
	if err != nil {
		respwrite.FailWithMessage(fmt.Sprintf("更新失败，%v", err), c)
	} else {
		respwrite.OkWithData(response.SysAuthorityResponse{Authority: authority}, c)
	}
}
...
func GetAuthorityList(c *gin.Context) {
	...
	err, list, total := sys_service.GetAuthorityInfoList(pageInfo)
	if err != nil {
...
func SetDataAuthority(c *gin.Context) {
	...
	err := sys_service.SetDataAuthority(auth)
	if err != nil {
		respwrite.FailWithMessage(fmt.Sprintf("设置关联失败，%v", err), c)
	} else {
		respwrite.Ok(c)
	}
}
